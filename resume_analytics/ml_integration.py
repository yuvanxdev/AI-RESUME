...
from .ml_models import ResumeMLModel
...
class MLResumeAnalyzer:
    """Integration class that combines traditional resume analysis with ML-based scoring"""
    
    def __init__(self):
        """Initialize the ML-enhanced resume analyzer"""
    ...
    def _ensure_models_exist(self):
        """Ensure that ML models are available"""
        model_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models')
        model_files = [
            os.path.join(model_dir, 'ats_score_model.pkl'),
            os.path.join(model_dir, 'format_score_model.pkl'),
            os.path.join(model_dir, 'section_score_model.pkl'),
            os.path.join(model_dir, 'tfidf_vectorizer.pkl')
        ]
        
        # Check if all model files exist
        if not all(os.path.exists(file) for file in model_files):
            logger.info("Some ML models are missing. Training new models...")
            self.train_models()
    
    def train_models(self, dataset_path=None):
        """Train ML models for resume scoring"""
        logger.info("Training ML models for resume scoring")
        self.ml_model.train_models(dataset_path)
    
    def analyze_resume(self, resume_text, job_description=None):
        """Analyze resume using ML models and return enhanced scores"""
        # Get ML-based scores
        ml_scores = self.ml_model.predict_scores(resume_text, job_description)
        ...
        analysis_results = {
            ...
            "ats_score": ml_scores["ats_score"],
            "format_score": ml_scores["format_score"],
            "section_score": ml_scores["section_score"],
            "keyword_match": {
...
if __name__ == "__main__":
    # Create the ML-enhanced analyzer
